...
export default function Actions({conta}){
   const acoes = ['Área_Pix', 'Recarga', 'Empréstimo','Cobrar', 'Transferir', 'Pagar']
      ...
      <>
      {
         acoes.map((acao,index) => (
            <span className="flex flex-col items-center gap-4" key={index}>
               <Icon icon={acao}  title={acao.split('_').join(' ')} conta={typeof conta != 'object' ? valor(Math.floor(conta.replace(/\D/g, '')) * 1.5) : conta}
